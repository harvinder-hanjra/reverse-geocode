 ...
 * main.js — S2 Atlas.
 *
 * Offline world map backed by the S2/RGEO0001 reverse geocoder.
 * Hover for instant administrative hierarchy lookup via s2_geo.bin.
 */
...
Promise.all([
  ...
  fetchBuffer(`${DATA}/s2_geo.bin`, (r, t) => { prog[1] = { r, t }; updateBar(); })
    .then(buf => new S2(buf)),
]).then(([gj, s2inst]) => {
...
function frame() {
  rafPending = false;
  if (s2 && mapReady) runQuery(pendingX, pendingY);
}
...
function runQuery(cx, cy) {
  ...
  const adminId = s2.lookup(lat, lng);
  const ms      = performance.now() - t0;
  const coords  = `${fmtCoord(lat, 'N', 'S')}  ${fmtCoord(lng, 'E', 'W')}`;

  // Highlight via rendered feature (robust regardless of admin_id alignment)
